Shader "Custom/ShaderMaterial_"
{
     Properties
    {
		...
		PrincipledBSDF_Metallic("Metallic", float) = 0.4509090781211853
		PrincipledBSDF_Specular("Specular", float) = 0.5
		PrincipledBSDF_SpecularTint("SpecularTint", float) = 0.0
		PrincipledBSDF_Roughness("Roughness", float) = 0.20545455813407898
...
float PrincipledBSDF_EmissionStrength,float PrincipledBSDF_Alpha, float3 PrincipledBSDF_Normal,float3 PrincipledBSDF_ClearcoatNormal, float3 PrincipledBSDF_Tangent, float PrincipledBSDF_Weight)
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float3 ImageTexture001_Vector = mapping(Mapping_Vector, Mapping_Location, Mapping_Rotation, Mapping_Scale);
				float4 PrincipledBSDF_BaseColor = image_texture(ImageTexture001_Vector, ImageTexture001_Image);
