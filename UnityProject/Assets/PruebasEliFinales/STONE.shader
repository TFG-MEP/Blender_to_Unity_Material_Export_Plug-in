Shader "Custom/ShaderSTONE_"
{
     Properties
    {
		...
		PrincipledBSDF_SubsurfaceColor("SubsurfaceColor", Color) = (1.0,1.0,1.0, 1.0)
		...
		PrincipledBSDF_Specular("Specular", float) = 0.25909093022346497
...
float PrincipledBSDF_EmissionStrength,float PrincipledBSDF_Alpha, float3 PrincipledBSDF_Normal,float3 PrincipledBSDF_ClearcoatNormal, float3 PrincipledBSDF_Tangent, float PrincipledBSDF_Weight)
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float PrincipledBSDF_Roughness = float3_to_float(Image_Texture_001.Color);
				float3 Imagen_Vector = float3(i.uv,0);
				Image_Texture_struct Imagen = image_texture(Imagen_Vector, Imagen_Image);
				float3 NormalMap_Color = Imagen.Color;
