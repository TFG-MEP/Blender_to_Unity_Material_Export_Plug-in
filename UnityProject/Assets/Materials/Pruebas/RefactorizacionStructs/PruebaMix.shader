Shader "Custom/ShaderPruebaMix_"
{
     Properties
    {
        _NormalTex("Normal Map", 2D) = "bump" {}
        MixShader_Fac("Fac", float) = 0.8933333158493042
		...
		Mix005_Factor_Float("Factor_Float", float) = 0.5
		Mix005_Factor_Vector("Factor_Vector", Vector) = (0.5, 0.5, 0.5)
		Mix005_A_Float("A_Float", float) = 0.0
		Mix005_B_Float("B_Float", float) = 0.0
		Mix005_A_Vector("A_Vector", Vector) = (0.0, 0.0, 0.0)
		Mix005_B_Vector("B_Vector", Vector) = (0.0, 0.0, 0.0)
		Mix005_A_Color("A_Color", Color) = (0.7297400528407231,0.0,0.07796869023772526, 1.0)
		Mix005_B_Color("B_Color", Color) = (0.0,0.7297400528407231,0.08968364329795708, 1.0)
		Mix_005_Clamp_Factor("Clamp_Factor", Int) = 1
		Mix_005_Clamp_Result("Clamp_Result", Int) = 0
...
{ \
    ...
    c -= rot(b, 24); \
			...
			float Mix005_Factor_Float;
			float3 Mix005_Factor_Vector;
			float Mix005_A_Float;
			float Mix005_B_Float;
			float3 Mix005_A_Vector;
			float3 Mix005_B_Vector;
			float3 Mix005_A_Color;
			float3 Mix005_B_Color;
			bool Mix_005_Clamp_Factor;
			bool Mix_005_Clamp_Result;
...
float PrincipledBSDF_EmissionStrength,float PrincipledBSDF_Alpha, float3 PrincipledBSDF_Normal,float3 PrincipledBSDF_ClearcoatNormal, float3 PrincipledBSDF_Tangent, float PrincipledBSDF_Weight)
        ...
        mixed.Shader.w = clamp(mixed.Shader.w, 0.0, 1.0);
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				Mix_color_struct Mix_005 = mix_color(Mix005_Factor_Float, Mix005_Factor_Vector, Mix005_A_Float, Mix005_B_Float, Mix005_A_Vector, Mix005_B_Vector, Mix005_A_Color, Mix005_B_Color, Mix_005_Clamp_Factor, Mix_005_Clamp_Result);
				float3 PrincipledBSDF_BaseColor = Mix_005.Result;
				...
				Mix_Shader_struct Mix_Shader = mix_shader(MixShader_Fac, MixShader_Shader, MixShader_Shader_001);
