Shader "Custom/ShaderNoFunciona_"
{
     Properties
    {
        _NormalTex("Normal Map", 2D) = "bump" {}
        Mix002_Factor_Float("Factor_Float", float) = 0.20000000298023224
		...
		VoronoiTexture_Scale("Scale", float) = 5.0
...
float PrincipledBSDF_EmissionStrength,float PrincipledBSDF_Alpha, float3 PrincipledBSDF_Normal,float3 PrincipledBSDF_ClearcoatNormal, float3 PrincipledBSDF_Tangent, float PrincipledBSDF_Weight)
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float3 Mix002_B_Color = Voronoi_Texture.Color;
				...
				float3 PrincipledBSDF_Emission = Mix_002.Result;
				...
				float4 MixShader_Shader = Principled_BSDF.BSDF;
				// Call methods
