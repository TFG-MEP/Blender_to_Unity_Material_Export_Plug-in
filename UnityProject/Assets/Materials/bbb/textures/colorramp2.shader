Shader "Custom/Shadercolorramp2_"
{
     Properties
    {
		...
		ColorRamp_Pos1("ColorRamp_Pos1", Float) = 0.4500000774860382
		ColorRamp_Color2("ColorRamp_Color2", Color) = (0.6474567704192329,0.1470427699182262,0.032829007155875486, 1.0)
		ColorRamp_Pos2("ColorRamp_Pos2", Float) = 0.8409091830253601
		// Add properties
			...
			float4 color_ramp( float at,int numcolors, int interpolate,float4 ramp[30],float pos[30] )
{
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float4 MaterialOutput_Surface = color_ramp(ShadertoRGB_Shader, 3, 1, ColorRamp_ramp, ColorRamp_pos);
				// Call methods
